...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 // Note, transposing the out degree matrix yields the indegree matrix
		 // So, out degree = getNeighbors; in degree = getInNeighbors

		 List<Integer> returnValue = new ArrayList<Integer>();
		 List<Integer> outNeighbors = this.getNeighbors(v);

		 for (int iElement : outNeighbors) {
			 for (int j = 0; j < this.getNumVertices(); j++) {
				 List<Integer> inNeighbors = this.getInNeighbors(j);

				 if (inNeighbors.contains(iElement)) {
					 returnValue.add(j);
				 }
			 }
		 }
